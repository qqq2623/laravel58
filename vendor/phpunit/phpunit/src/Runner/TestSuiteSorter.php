...
private
	...
	public function __construct(
        ...
        public static function getTestSorterUID(Test $test): string {
	        ...
	        return $test->getName();
        }?TestResultCacheInterface $cache = null)
    {
 ...
 */
public
function reorderTestsInSuite(Test $suite, int $order, bool $resolveDependencies, int $orderDefects, bool $isRootTestSuite = true): void {
	$allowedOrders = [
		self::ORDER_DEFAULT,
		self::ORDER_REVERSED,
...
public
function getOriginalExecutionOrder(): array {
	return $this->originalExecutionOrder;
}

public
function getExecutionOrder(): array {
	return $this->executionOrder;
}

private
function sort(TestSuite $suite, int $order, bool $resolveDependencies, int $orderDefects): void {
...
private
function addSuiteToDefectSortOrder(TestSuite $suite): void {
	$max = 0;
...
private
function suiteOnlyContainsTests(TestSuite $suite): bool {
	return \array_reduce(
		$suite->tests(),
		function ($carry, $test) {
...
private
function reverse(array $tests): array {
	return \array_reverse($tests);
}

private
function randomize(array $tests): array {
	\shuffle($tests);
...
private
function sortDefectsFirst(array $tests): array {
	\usort(
		$tests,
		function ($left, $right) {
...
private
function sortByDuration(array $tests): array {
	\usort(
		$tests,
		function ($left, $right) {
 ...
 */
private
function cmpDefectPriorityAndTime(Test $a, Test $b): int {
 ...
 */
private
function cmpDuration(Test $a, Test $b): int {
 ...
 */
private
function resolveDependencies(array $tests): array {
	$newTestOrder = [];
	$i            = 0;
 ...
 */
private
function getNormalizedDependencyNames($test): array {
...
private
function calculateTestExecutionOrder(Test $suite): array {
	$tests = [];
